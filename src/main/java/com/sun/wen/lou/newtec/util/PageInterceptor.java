...
public class PageInterceptor implements Interceptor {
	...
	private static final ThreadLocal<PageController> PAGE_CONTEXT = new ThreadLocal<PageController>();
...
//			Reflections.setValueByFieldName(boundSql, "parameterObject", changeCode(param));
			if (param instanceof HashMap) {
				// 参数中含有page就进行分页处理
				HashMap map = (HashMap) param;
				if(map.containsKey(PAGE_KEY)) {
					PageController p = (PageController) map.get(PAGE_KEY);
					if (p != null) {
						p.setRowsAmountSet(false);
						p.setTotalRowsAmount(queryTotal(invocation, mappedStatement, boundSql, param, originalSql));
						if(p.getToPage()==0) {
							p.setToPage(1);
						}
						p.setCurrentPage(p.getToPage());
...
//				try {
//					HashMap map=(HashMap)BeanUtils.describe(bean);
...
//				}
	 ...
	 * @return
	 */
	private String pageSql(String originalSql, PageController page) {
		...
		if ("mysql".equals(dialect)) {
			...
			int start = page.getPageStartRow()>0?page.getPageStartRow()-1:page.getPageStartRow();
			sb.append(" limit " +  start + "," + page.getPageSize());
		} else if ("oracle".equals(dialect)) {
			sb.append("select * from (select pagetable.*,ROWNUM AS rowcounter from (");
			sb.append(originalSql);
			sb.append(")  pagetable) subt where subt.rowcounter>");
			sb.append(page.getPageStartRow() - 1);
			sb.append(" AND subt.rowcounter<=");
			sb.append(page.getPageEndRow());
			sb.append(" and rownum <="+page.getPageSize());//解决海量数据分页
